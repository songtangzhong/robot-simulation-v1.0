/*
    ...
    It will write data into shared memory for one minute.
    This node must be running before reading operation.
*/
...
int main(int argc, char ** argv)
{
    ...
    while (rclcpp::ok())
    {
        ...
        {
            shm_ptr->cur_joint_positions_[j] = j;
            shm_ptr->cur_joint_velocities_[j] = j;
            shm_ptr->cur_joint_efforts_[j] = j;

            shm_ptr->cmd_joint_positions_[j] = j;
            shm_ptr->cmd_joint_velocities_[j] = j;
            shm_ptr->cmd_joint_efforts_[j] = j;

            shm_ptr->control_mode_[j] = j;
        }
