...
1. Description:
```
This package generates a shared library "libgazebo_plugin.so", 
which is a gazebo plugin communicating with shared memory.
To use the shared library, add following command to your ~/.bashrc:
...
2. Description:
```
test/test_gazebo_plugin_write.cpp is used to test the gazebo control plugin
